...
class FileToTXT:
    ...
    def __init__(self, filepath: str):
        ...
        if filepath.endswith('.txt'):
            self.text = self.__parse_txt(filepath)
            self.filename = basename(filepath)[:-4]
        elif filepath.endswith('.epub'):
            self.text = self.__parse_epub(filepath)
            self.filename = basename(filepath)[:-5]
        elif filepath.startswith("http"):
            self.text = self.__parse_online_html(filepath)
            cleaned_url = sub(r'https?://', '', filepath)
            self.filename = sub(r'\W', '_', cleaned_url)
        elif filepath.endswith('.html'):
            self.text = self.__parse_local_html(filepath)
            self.filename = basename(filepath)[:-5]
        elif filepath.endswith('.pdf'):
            self.text = self.__parse_pdf(filepath)
            self.filename = basename(filepath)[:-4]
        elif filepath.endswith('.docx'):
            self.text = self.__parse_docx(filepath)
            self.filename = basename(filepath)[:-5]
    ...
    def __parse_epub(self, filepath: str) -> list:
